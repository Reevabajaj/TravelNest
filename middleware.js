...
module.exports.isloggedin=(req,res,next)=>{
    if(!req.isAuthenticated()){
        req.session.redirecturl=req.originalUrl;//req.session ke pas session ke sari details hote ha.usi ke andar we'll create a new parameter redirecturl.
        req.flash("error","You need to log in to use this feature ");
        return res.redirect("/login");//return is liye kra kioki taki neeche necyt pe ne ajaye ab sidha login page pe jaye
     }
     next();//if user authenticated then next
}
